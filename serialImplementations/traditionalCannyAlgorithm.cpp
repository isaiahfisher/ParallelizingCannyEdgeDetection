...
void EdgeDetection(Mat input, Mat output, low, high, sigma)
{
    ...
    Sobel(imgBlurred, xGradient, CV_64F, 1, 0, ksize=3);
    Sobel(imgBlurred, yGradient, CV_64F, 0, 1, ksize=3);
    ...
    for (int i = imgBlurred.cols - 1; i > 0; i--)
    {
        for (int j = imgBlurred.rows - 1; j > 0; j--)
        {
            if (bottom_right.at<uchar>(j,i) == 100)
            {
                try
                {
                    if (bottom_right.at<uchar>(j+1, i) == 255
                            || bottom_right.at<uchar>(j-1,i) == 255
                            || bottom_right.at<uchar>(j+1, i-1) == 255
                            || bottom_right.at<uchar>(j+1, i+1) == 255
                            || bottom_right.at<uchar>(j-1, i-1) == 255
                            || bottom_right.at<uchar>(j-1, i+1) == 255
                            || bottom_right.at<uchar>(j, i-1) == 255
                            || bottom_right.at<uchar(j, i+1) == 255)
                    {
